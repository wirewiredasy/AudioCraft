...
export const NoiseReducer: React.FC = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Failed to reduce noise')
    } finally {
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">
              Noise Reduction Strength: {noiseReductionStrength}
            </label>
            <Input
              type="range"
          ...
          {result && (
            <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-800 font-medium mb-2">Noise Reduction Complete!</p>
